...
async function getList(url: string): Promise<unknown[]> {
  try {
...
export const api = {
  ...
  getStudySessions: () => getList('/api/study-sessions'),
  ...
  getPracticeLogs: () => getList('/api/practice-logs'),
  ...
  getOtherTestScores: () => getList('/api/higher-study/test-scores'),
  ...
  getScholarships: () => getList('/api/higher-study/scholarships'),
  ...
  getTemplates: () => getList('/api/higher-study/templates'),
  ...
  getPlanningNotes: (section?: string) => getList(`/api/planning-notes${section ? `?section=${section}` : ''}`),
